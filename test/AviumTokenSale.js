...
contract('AviumTokenSale', function(accounts){
  ...
  it('facilitates token buying', function() {
    return AviumToken.deployed().then(function(instance) {
      // Grab token instance first
      tokenInstance = instance;
      return AviumTokenSale.deployed();
    ...
    return AviumToken.deployed().then(function(instance) {
      ...
      balance = web3.eth.getBalance(tokenSaleInstance.address).then(resp =>{
        assert.equal(resp.toNumber(), 0)
      })
    });
  });
});
